#!/usr/bin/env bash
#
# Legt den Vapi-Assistenten für ein Restaurant an (oder aktualisiert ihn) und
# verknüpft ihn mit dessen Vapi-Telefonnummer.
# Aufruf: bash setup-vapi.sh <restaurant-id>
# Voraussetzungen: install.sh wurde ausgeführt (/etc/ki-works/ki-works.env
# existiert), das Restaurant existiert bereits in der DB mit Name, Adresse
# und vapi_phone_number (z. B. über POST /api/restaurants bzw. das Dashboard
# "Kunden (Betreiber)" angelegt).
#
set -euo pipefail
...
RESTAURANT_JSON=$(curl -sS "http://127.0.0.1:3001/api/restaurants" \
  | jq -r --arg id "$RESTAURANT_ID" '.[] | select((.id|tostring) == $id)')
[[ -n "$RESTAURANT_JSON" ]] || { echo "Restaurant mit ID $RESTAURANT_ID nicht gefunden."; exit 1; }

RESTAURANT_NAME=$(echo "$RESTAURANT_JSON" | jq -r '.name')
RESTAURANT_PHONE=$(echo "$RESTAURANT_JSON" | jq -r '.vapi_phone_number // empty')
EXISTING_ASSISTANT_ID=$(echo "$RESTAURANT_JSON" | jq -r '.vapi_assistant_id // empty')
...
if [[ -n "$EXISTING_ASSISTANT_ID" ]]; then
  ...
  echo "==> Neuen Vapi-Assistenten 'ki-works – $RESTAURANT_NAME' anlegen..."
...
if [[ -n "$RESTAURANT_PHONE" ]]; then
  echo "==> Telefonnummer $RESTAURANT_PHONE mit dem Assistenten verknüpfen..."
  PHONE_ID=$(curl -sS https://api.vapi.ai/phone-number \
    -H "Authorization: Bearer $VAPI_API_KEY" \
    | jq -r --arg num "$RESTAURANT_PHONE" '.[] | select(.number == $num) | .id')
  ...
  if [[ -n "$PHONE_ID" ]]; then
    ...
    echo "WARNUNG: Nummer $RESTAURANT_PHONE nicht im Vapi-Konto gefunden — im Vapi-Dashboard manuell verknüpfen."
  fi
else
  echo "WARNUNG: Kein vapi_phone_number für Restaurant $RESTAURANT_ID hinterlegt — Nummer zuerst im Dashboard/DB eintragen, dann Skript erneut laufen lassen."
fi

echo "==> Assistant-ID in der Datenbank speichern..."
curl -sS -X PATCH "http://127.0.0.1:3001/api/restaurants/$RESTAURANT_ID" \
  -H "Content-Type: application/json" \
  -d "{\"vapi_assistant_id\": \"$ASSISTANT_ID\"}" | jq '{id, name, vapi_assistant_id}'

echo ""
echo "FERTIG! Testanruf: ${RESTAURANT_PHONE:-<Nummer noch nicht verknüpft>}"
